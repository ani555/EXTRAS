...
import math
from attention import ScaledDotProductAttention, MultiHeadedAttention, AttentionWithCoverage
import numpy as np
...
class PointerGenerator(nn.Module):
    ...
    def forward(self, x, enc_outputs, src_mask, enc_batch_extend_vocab, extra_zeros):
        ...
        p_vocab = F.softmax(self.fc(torch.cat((x, cross_context), dim=-1)), dim=-1)
        p_gen = torch.sigmoid(self.p_gen_fc(torch.cat((x, cross_context), dim=-1))) #[BXTy]
        #print(p_gen.shape)
...
class PointerGeneratorWithCoverage(nn.Module):
    ...
    def forward(self, x, enc_outputs, src_mask, enc_batch_extend_vocab, extra_zeros):
        #cross_attn_op = cross_attn_op.permute(0,2,3,1).contiguous()
        #print(cross_attn_op.shape)
        cross_context1, cross_attn_op1 = self.single_head_attn1(querys=x, keys=enc_outputs, values=enc_outputs, mask=src_mask)
        attn_probs1 = cross_attn_op1.squeeze(1)
        # calculate the coverage vector here
        batch_size = attn_probs1.size(0)
        Ty = attn_probs1.size(1)
        Tx = attn_probs1.size(2)
        #print(batch_size, Tx, Ty)
        coverage_mask = torch.from_numpy(np.triu(np.ones((batch_size, Ty, Ty)), k=1).astype('uint8')).float().cuda() #TyxTy
        coverage = torch.bmm(torch.transpose(attn_probs1,1,2), coverage_mask).transpose(1,2) #[[BxTyxTx]'*[BxTyxTy]]' = [BxTyxTx]
        #print(x.shape, enc_outputs.shape, coverage.shape)
        cross_context2, cross_attn_op2 = self.single_head_attn2(x, enc_outputs, coverage, mask=src_mask)
        attn_probs2= cross_attn_op2.squeeze(1)
        p_vocab = F.softmax(self.fc(torch.cat((x, cross_context2), dim=-1)), dim=-1)
        p_gen = torch.sigmoid(self.p_gen_fc(torch.cat((x, cross_context2), dim=-1))) #[BXTy]
        #print(p_gen.shape)
        # if enc_batch_extend_vocab is not None:
        #     print("enc_batch_extend_vocab: ",enc_batch_extend_vocab.shape)
        attn_dist = (1-p_gen)*attn_probs2
        p_vocab = p_gen*p_vocab
        ...
        cov_loss = torch.min(attn_probs2, coverage).sum()
        #print(enc_batch_extend_vocab.shape)
        # print("p_vocab_extended: ",p_vocab_extended.shape)
        return torch.log(torch.clamp(p_vocab_extended, min = 1e-9)), cov_loss
