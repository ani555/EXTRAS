...
class BeamSearchDecoder:
    ...
    def __init__(self, model):
        ...
        self._decode_dir = os.path.join(config.log_root, 'decode_%s' % ("model2"))
    ...
    def beam_search(self, batch, conf):
        ...
        while steps < config.max_dec_len and len(results) < config.beam_size:
            ...
            if extra_zeros is not None:
                extra_zeros_ip = extra_zeros[:, 0:steps + 1, :].repeat(yt.size(0), 1, 1)
            if conf.coverage:
                log_probs,_ = self.model.generator(out, temp_enc_out, enc_padding_mask, enc_batch_extend_vocab,
                                           extra_zeros_ip)
            else:
