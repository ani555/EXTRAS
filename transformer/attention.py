...
class ScaledDotProductAttention2(nn.Module):
    ...
    def forward(self, querys, keys, values, coverage, mask=None):
        
        # querys, keys, values - [BxnheadsxTxP]
        dk = keys.size(-1)
        attn_scores = (torch.matmul(querys, torch.transpose(keys, 2, 3)) + coverage.unsqueeze(1))/math.sqrt(dk)
...
class AttentionWithCoverage(nn.Module):
    ...
    def __init__(self, d_model, encoder_seq_length, nheads=1, drop_prob=0.2):
        ...
        self.val_layer = nn.Linear(d_model, d_model)
        self.output_layer = nn.Linear(d_model, d_model)
        self.attn = ScaledDotProductAttention2(drop_prob)
        self.coverage_proj = nn.Linear(encoder_seq_length, encoder_seq_length)

    def forward(self, querys, encoder_states, coverage_vectors, mask=None):
        batch_size = querys.size(0)
        coverage =  self.coverage_proj(coverage_vectors)
        keys=values = encoder_states
        proj_querys = self.query_layer(querys).view(batch_size, -1, self.nheads, self.d_k).transpose(1,2)  # [BxnheadsxTxP]
        proj_keys = self.key_layer(keys).view(batch_size, -1, self.nheads, self.d_k).transpose(1, 2)  # [BxnheadsxTxP]
        proj_values = self.val_layer(values).view(batch_size, -1, self.nheads, self.d_k).transpose(1,2)  # [BxnheadsxTxP]

        context, attn_outputs = self.attn(proj_querys, proj_keys, proj_values, coverage,
                                          mask)  # context [BxnheadsxTxd_model] attn_outputs [BxnheadsxTxT]
        context = torch.transpose(context, 1, 2).contiguous().view(batch_size, -1,
                                                                   self.nheads * self.d_k)  # [BxTxd_model]
